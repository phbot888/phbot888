	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} ja foi tarde corno😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: '⚠️ Falha, ocorreu um erro ao converter a imagem em figurinha fale com o corno do ph pra resolver⚠️',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '⚠️ Este comando só pode ser usado pelo dono do bot!PH GOSTOSZAUM 😂',
			...
			const ownerNumber = ["5568999638546@s.whatsapp.net"] // replace this with your number
